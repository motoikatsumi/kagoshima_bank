#!/usr/bin/env python3
"""
鹿児島銀行 FB-Web 取引履歴取得 → Slack報告スクリプト
...
def normalize_key(date_str, name_str):
    """日付と摘要を正規化してキーを作成"""
    import re
    # 日付を正規化: スペース・全角スペース除去
    date_clean = date_str.strip().replace("\u3000", "").replace(" ", "")
    # 名前を正規化: 前後スペース、全角スペース、NBSP除去
...
def extract_reported_keys(channel_id, limit=50):
    ...
    for msg in messages:
        ...
        for line in text.split("\n"):
            ...
            # 日付パターン: 2026/03/31 または 2026/04/03 で始まる行
            ...
            # パイプ区切り（新スクリプト形式）
            # 例: 2026/03/31 | ｼｵﾀ ﾄﾓｺ | 入金 20,925円 | 残高 ...
            if "|" in rest:
                parts = rest.split("|")
                if len(parts) >= 2:
                    name = parts[1].strip()
            ...
            # タブ区切り（Claude/手動投稿形式）
            # 例: 2026/04/03\tﾀﾆ ｺｳｽｹ\t\t2,700円\t...
            # 摘要なしの場合: 2026/04/03\t\t\t4,121,000円\t3,075,788円
            if "\t" in rest:
                parts = rest.split("\t")
                # 最初の非空・非金額要素が摘要
                name_found = False
...
def get_password_from_keychain():
    """macOSキーチェーンから銀行パスワードを取得"""
    try:
        result = subprocess.run(
            ["security", "find-generic-password", "-a", "kagin_assist", "-s", "kagin_bank_login", "-w"],
            capture_output=True, text=True, timeout=10,
        )
        if result.returncode == 0:
            password = result.stdout.strip()
            if password:
                logger.info("キーチェーンからパスワードを取得しました")
                return password
        logger.warning("キーチェーンにパスワードが見つかりません")
        return None
    except Exception as e:
        logger.error(f"キーチェーンアクセスエラー: {e}")
        return None
...
def login_to_bank(driver):
    ...
    try:
        # ログインページかどうか確認（契約法人IDフィールドの存在チェック）
        ...
        if password:
            ...
            for by, selector in selectors:
                try:
                    ...
                    logger.info(f"ログインボタン発見: {by}={selector}, tag={btn.tag_name}, value={btn.get_attribute('value')}, text={btn.text}")
                    btn.click()
                    login_clicked = True
                    break
            ...
            # CORP_IDがまだ存在するかチェック（implicit waitを一時的に短縮）
            ...
            try:
                ...
                logger.info("ログイン失敗スクリーンショット保存: login_failed.png")
                return False
            ...
            # キーチェーンにパスワードがない場合 → 手動入力を待つ
            logger.info("=" * 50)
            logger.info("キーチェーンにパスワードがありません")
            logger.info("ブラウザでパスワードを入力して「ログイン」を押してください")
            logger.info("（最大120秒待機）")
            logger.info("=" * 50)
            ...
            for i in range(120):
                ...
                try:
                    ...
                    logger.info("ログイン成功を検出しました")
                    time.sleep(3)
                    return True

            logger.warning("ログイン待機タイムアウト（120秒）")
            return False

    except TimeoutException:
        # ログインページではない（すでにログイン済み、またはセッション有効）
                ...
                try:
                    ...
                    for link in links:
                        ...
                        if "入出金明細" in text or "明細照会" in text:
                            # JavaScriptでクリック（click intercepted回避）
                            try:
...
def try_select_by_text(select_elem, value):
    """ドロップダウンの値を複数の形式で試す（例: '4', '04', ' 4'）"""
    val = int(value)
    formats = [str(val), f"{val:02d}", f" {val}", f"{val} "]
    for fmt in formats:
        try:
            select_elem.select_by_visible_text(fmt)
            return True
        except Exception:
            continue
    # 最後の手段: indexで選択
    try:
        select_elem.select_by_value(str(val))
        return True
    except Exception:
        pass
    try:
        select_elem.select_by_value(f"{val:02d}")
        return True
    except Exception:
        pass
    logger.warning(f"ドロップダウンで値 {value} を選択できません")
    return False


def set_date_range(driver):
    """照会期間を設定（前日〜今日の2日間）- JavaScript一括設定"""
    today = datetime.now()
    yesterday = today - timedelta(days=1)

    try:
        # JavaScriptで一括設定（ヘッドレスモードでの安定性向上）
...
def select_branch(driver, branch_index):
    """店舗を選択（ページが遷移していたら入出金明細照会ページに戻る）"""
    try:
...
def click_inquiry_button(driver):
    """表示条件変更（doInquire）を実行して日付フィルターを適用"""
    try:
        # 方法1: doInquire()をJavaScriptで直接実行
...
def format_transaction_message(branch_name, new_transactions):
    """Slackに送信するメッセージを整形（以前の形式: タブ区切り）"""
    lines = ["```"]
...
def report_to_slack(branch_name, channel_id, data_rows):
    ...
    logger.info(f"{branch_name}: メッセージ長={len(message)}文字, 先頭100文字={message[:100]}")
...
def slack_connection_test():
    ...
    try:
        ...
        data = resp.json()
        logger.info(f"auth.test応答: ok={data.get('ok')}, user={data.get('user')}, team={data.get('team')}, error={data.get('error')}")
        if not data.get("ok"):
...
def main():
    logger.info("=" * 60)
    logger.info("鹿児島銀行 Slack報告スクリプト開始")
    logger.info("=" * 60)

    # Slack接続テスト
    if not slack_connection_test():
        logger.error("Slack接続に失敗しました。トークンを確認してください。")
        # Slack自体が繋がらないので通知不可
        return
    ...
    try:
        ...
        if not login_to_bank(driver):
            error_msg = "銀行ログインに失敗しました。パスワードまたはアカウント状態を確認してください。"
        ...
        if not nav_ok:
            error_msg = "入出金明細照会ページへの遷移に失敗しました。銀行サイトの状態を確認してください。"
            logger.error(error_msg)
            slack_notify_error(error_msg)
        ...
        # 全店舗でデータ取得できなかった場合に通知
        if success_count == 0:
            error_msg = f"全{len(BRANCHES)}店舗でデータ取得に失敗しました。銀行セッションまたはページ遷移の問題の可能性があります。"
            logger.error(error_msg)
            slack_notify_error(error_msg)

    except Exception as e:
        logger.error(f"致命的エラー: {e}")
        slack_notify_error(f"致命的エラー: {e}")
